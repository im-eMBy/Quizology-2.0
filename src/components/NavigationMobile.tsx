...
export const NavigationMobile: React.FC = () => {
    ...
    const handleNavClick = (page: "Play" | "Suggest" | "Admin") => {
        appSetPage(page);
        setMenuOpen(false);
    }

    const getNavigationList = (): JSX.Element => {
        return <ul className="navigation__mobile-list" style={menuOpen ? { top: "70px", visibility: "visible" } : { top: "-100%", visibility: "hidden" }}>
            <li className="navigation__element">
                <button onClick={() => handleNavClick("Play")}>Graj</button>
            </li>
            <li className="navigation__element">
                <button onClick={() => handleNavClick("Suggest")}>Zaproponuj pytanie</button>
            </li>
        </ul>
    }
